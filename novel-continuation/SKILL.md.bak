...
7. **提取前5章大纲摘要** - 从 `design/01-大纲.md` 提取当前章节及前5章的大纲规划（核心事件、悬念钩子、章节意图、伏笔回收），整理为审计基线，供流转前的33维度审计使用。
...
1. **评审文件存在检查**：确认 `chapters/_review-第{XX}章.md` 存在且内容非空。不存在则回到步骤 3-A 补评审。
...
3. **33维度审计（对照近5章大纲）**：使用写前分析步骤7提取的前5章大纲摘要作为基线，对本章执行33维度审计，**重点检查以下维度**：
   - **大纲一致性**：本章实际内容是否遵循 `design/01-大纲.md` 中规划的核心事件、章节意图？未完成规划事件则标记为不合格。
   - **悬念钩子承启**：本章结尾钩子是否承接上章悬念、并为下章埋钩？与大纲规划的悬念链路一致？
   - **人物弧线推进**：本章人物发展是否按大纲规划的弧线阶段推进？无跳跃无倒退？
   - **主线/支线进度**：本章是否按大纲推进了主线/支线？如偏离规划需在 `design/98-写作决策日志.md` 记录理由。
   - **伏笔回收节奏**：本章是否按大纲的预期回收/设置了伏笔？债务表中对应项已更新？
   - **跨章设定一致**：本章设定与前5章+约束文档无矛盾（人物OOC、时间线跳跃、战力崩坏、术语拼写不一等）。
   - **字数达标评估**：确认字数 ≥ 2500，且内容充实度与大纲规划的章节权重匹配（高潮章不应短于过渡章）。
   - **33维度其他维度**：参考第5步质量循环的33维度清单，按题材激活对应维度。
   
   **审计结果写入 `chapters/_review-第{XX}章.md` 的「33维审计」区块**（追加到现有评审文件末尾）。审计不通过则阻塞流转，回到步骤 2 修订本章，修订后重新执行步骤 3（撰写后优化）→ 步骤 3-A（章节评审门）→ 步骤 4（收尾）→ 再次执行流转前断言。
